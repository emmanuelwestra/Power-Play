...
void displayBattle(const Object& player, const std::vector<Object>& monsters)
{
    ...
    for_each(player.inventory.begin(), player.inventory.end(), [](const auto& item) {
        std::cout << "  ";
    ...
    for_each(monsters.begin(), monsters.end(), [](const Object& monster) {
        std::cout << "   ";
        printName(monster);
...
void levelUp(Object& player)
{
    ...
    if (haveOne == player.inventory.end() || player.inventory[found.clasification].bonusValue < found.bonusValue)
    {
